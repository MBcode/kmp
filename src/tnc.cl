...
(defun sv-al_ (i al) 
  (let ((all (collect-if #'listp al)))
    (mapcar_ #'(lambda (pr) (let ((sn (first-lv pr))) 
                              (when sn (sv_ i sn (cdr pr)))))
          all)))
;also have safe_al in u2
(defun sv_al_ (i al) 
  (sv_al i (collect-if #'listp al)))
...
(defun lt-assert (lf tf fn &optional (ct nil))
  "km assert" ;will parse lf more too
   ...
   (sv-al_ tf (mapcar #'(lambda (tg) (mk-si lf tg)) '("img" "i" "strong"))) ;helps assert :seq ;o25.kb*
   ;(sv_al_ tf (mapcar #'(lambda (tg) (mk-si lf tg)) '("img" "i" "strong"))) ;helps assert :seq;o26.kb
    ;;svs-al tf 
    #+ignore
    (sv-al tf 
